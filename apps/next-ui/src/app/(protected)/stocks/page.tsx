...
export default function StocksDashboard() {
  ...
  const fetchStocksData = useCallback(async (isBackgroundRefresh: boolean = false) => {
    ...
    try {
      ...
      const historyJson: StocksHistoryResponse = await historyRes.json();
      const stateJson: StocksStateResponse = await stateRes.json();

      if (historyJson.initializing) {
        setIsPolling(true);
      } else {
        setIsPolling(false);
      }
  ...
  }, [setSyncOptions, fetchStocksData, setLastSyncedText, isPolling]);
