...
export type EventRunnerConfig = {
  ...
  /** Async handler logic. Can return custom next run timestamp in ms */
...
export class ScheduledRunner {
  ...
  async start(): Promise<void> {
    ...
    try {
      // 1. Query persistent schedule state from Postgres
      let schedule = await db.workerSchedule.findUnique({
      ...
      if (!schedule) {
        schedule = await db.workerSchedule.create({
          data: {
      ...
      if (
        ...
        const targetTimeStr = schedule.nextRunAt.toISOString();
      } else if (
