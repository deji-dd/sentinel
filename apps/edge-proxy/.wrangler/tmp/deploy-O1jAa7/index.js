...
// src/index.ts
var PROXY_SECRET_HEADER = "Proxy-Secret-Header";
var ALLOWED_API_PATHS = /* @__PURE__ */ new Set(["/api/assist-events"]);
var ALLOWED_API_METHODS = /* @__PURE__ */ new Set(["POST", "PATCH", "DELETE"]);
var UUID_PATH_RE = /^\/install\/([0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\.user\.js$/i;
async function verifyInstallLinkSignature(uuid, expiresAt, providedSignature, secret) {
...
__name(verifyInstallLinkSignature, "verifyInstallLinkSignature");
...
function normalizeBotOrigin(origin) {
  return origin.replace(/\/+$/, "");
}
__name(normalizeBotOrigin, "normalizeBotOrigin");
function getJsonLimit(env) {
...
__name(getJsonLimit, "getJsonLimit");
async function proxyInstall(request, env, url, uuid) {
  ...
  const upstream = await fetch(upstreamUrl, {
    ...
    headers: {
      ...
      "X-Assist-Client-IP": request.headers.get("CF-Connecting-IP") || "unknown",
...
var index_default = {
  async fetch(request, env) {
    ...
    if (ALLOWED_API_METHODS.has(request.method) && ALLOWED_API_PATHS.has(url.pathname)) {
