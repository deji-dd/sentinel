...
export const AdminConfig = forwardRef(
  (
    ...
    const [logChannelId, setLogChannelId] = useState(
      initialData?.log_channel_id || "",
    );
    const [selectedAdminRoles, setSelectedAdminRoles] = useState<string[]>(
      Array.isArray(initialData?.admin_role_ids)
        ? initialData.admin_role_ids
        : [],
    );
    ...
    const fetchConfig = async (silent = false) => {
      ...
      try {
        ...
        setSelectedAdminRoles(
          Array.isArray(data.admin_role_ids) ? data.admin_role_ids : [],
        );
    ...
    useEffect(() => {
      if (!config) return;
      const initialRoles = Array.isArray(config.admin_role_ids)
        ? [...config.admin_role_ids].sort()
        : [];
      const currentRoles = [...selectedAdminRoles].sort();

      const rolesChanged =
        JSON.stringify(initialRoles) !== JSON.stringify(currentRoles);
    ...
    const handleAddKey = async () => {
      ...
      setAddingKey(true);
      try {
        const res = await fetchWithFallback("/api/config/api-keys", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${sessionToken}`,
    ...
    if (loading)
      return (
        <LoadingScreen fullScreen={false} subMessage="Loading Admin Config" />
      );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
          ...
          <Card className="border-border/50 bg-card/50 backdrop-blur-sm hover:shadow-xl transition-shadow duration-300">
            <CardHeader>
              ...
              <CardDescription className="text-muted-foreground/80">
                Manage Torn API keys for guild operations. Keys are encrypted at
                rest.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  size="icon"
                  className="shrink-0 bg-primary hover:bg-primary/90 cursor-pointer"
                  disabled={addingKey || !newKey}
                >
                  {addingKey ? (
                    <TacticalLoader size="16" stroke="3" color="white" />
                  ) : (
              ...
              <div className="max-h-75 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
                {config?.api_keys?.map((key: ApiKey) => (
                  <div
                    key={key.id}
                    className="flex items-center justify-between p-3 rounded-lg border border-border/50 bg-secondary/5 hover:bg-secondary/10 transition-colors group"
                  >
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-mono text-foreground/60 leading-none">
                          ••••••••••••••••
                        </span>
                        {key.is_primary === 1 && (
                          <Badge
                            variant="secondary"
                            className="text-[8px] h-4 bg-primary/10 text-primary border-primary/20 font-bold px-1.5"
                          >
                            PRIMARY
                          </Badge>
                        )}
                      ...
                      <p className="text-[10px] text-muted-foreground uppercase tracking-tighter">
                        ...
                        <span className="text-foreground/80 font-bold">
                          {key.provided_by_name || `@${key.provided_by}`}
                        </span>
                      </p>
                    </div>
                    <Button
                      ...
                      onClick={() =>
                        setDeleteConfirm({
                          id: key.id,
                          isLast: config.api_keys.length === 1,
                        })
                      }
                    >
          ...
          <Card className="border-border/50 bg-card/50 backdrop-blur-sm hover:shadow-xl transition-shadow duration-300">
            <CardHeader>
              ...
              <CardDescription className="text-muted-foreground/80">
                Select roles and channels for bot operations. Access restricted
                to owners.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-3">
                ...
                <Select
                  ...
                  onValueChange={(v) => setLogChannelId(v === "none" ? "" : v)}
                >
                ...
                <p className="text-[10px] text-muted-foreground leading-relaxed">
                  Sentinel will transmit security alerts and audit logs to this
                  identified text channel.
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent
                    ...
                    align="start"
                  >
                    <DropdownMenuLabel className="text-[10px] font-black uppercase tracking-widest opacity-50 px-2 py-1.5 focus:bg-accent/50">
                      Available Roles
                    </DropdownMenuLabel>
                    <DropdownMenuSeparator />
                    <DropdownMenuCheckboxItem
          ...
          <AlertDialogContent className="bg-background border-border rounded-3xl">
            ...
            <AlertDialogFooter className="gap-2">
              ...
              <AlertDialogAction
                variant="destructive"
                onClick={() =>
                  deleteConfirm && performDeleteKey(deleteConfirm.id)
                }
              >
                Confirm Removal
              </AlertDialogAction>
