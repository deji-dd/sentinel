...
export const BotManagementConfig = forwardRef(({
  ...
  const fetchGuildData = async (silent = false) => {
    ...
    try {
      ...
      const [resGuilds, resUnconfigured] = await Promise.all([
        fetch(`${API_BASE}/api/admin/guilds`, { headers: { Authorization: `Bearer ${sessionToken}` } }),
        fetch(`${API_BASE}/api/admin/unconfigured-guilds`, { headers: { Authorization: `Bearer ${sessionToken}` } })
      ]);
      
      if (!resGuilds.ok || !resUnconfigured.ok) throw new Error("Failed to fetch guilds");
  ...
  const toggleModule = async (guildId: string, moduleId: string, isEnabled: boolean) => {
    try {
      ...
      const API_BASE = import.meta.env.VITE_API_URL || "http://localhost:3001";
      const res = await fetch(`${API_BASE}/api/admin/guilds/${guildId}/modules`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${sessionToken}`
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500 pb-12">
        ...
        <Card className="border-border/50 bg-card/50 backdrop-blur-sm shadow-xl hover:shadow-2xl transition-all">
          <CardHeader>
            <div className="flex items-center gap-3 mb-2">
              ...
              <CardTitle className="text-xl font-bold tracking-tight">Database Backup</CardTitle>
            </div>
            <CardDescription>Generate a point-in-time snapshot of the SQLite database and deliver it via encrypted Discord DM.</CardDescription>
          </CardHeader>
          <CardFooter>
            <Button 
        ...
        <Card className="border-border/50 bg-card/50 backdrop-blur-sm shadow-xl hover:shadow-2xl transition-all">
          <CardHeader>
            <div className="flex items-center gap-3 mb-2">
              ...
              <CardTitle className="text-xl font-bold tracking-tight">Deploy Commands</CardTitle>
            </div>
            <CardDescription>Force-refresh slash command registrations across all active guilds. Use after adding new modules or fixing command definitions.</CardDescription>
          </CardHeader>
          <CardFooter>
            <Button 
      ...
      <div className="space-y-4 pt-4">
        <div className="flex justify-between items-center px-2">
          <div className="flex items-center gap-3">
            <h2 className="text-2xl font-black tracking-tight uppercase italic text-foreground/90">Managed Guilds</h2>
            <Badge variant="outline" className="font-mono bg-background/50">{guilds.length}</Badge>
          </div>
          
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="gap-2 font-bold px-4 hover:bg-primary/10 hover:text-primary transition-colors">
                <Plus className="w-4 h-4" />
                Initialize Guild
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent className="w-[240px] shadow-2xl bg-card border-border/50">
               ...
               {unconfiguredGuilds.length > 0 ? (
                 ...
                 <div className="px-2 py-4 text-xs text-muted-foreground italic text-center">No new guilds found</div>
               )}
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {guilds.map(guild => (
            <Card key={guild.id} className="border-border/40 bg-card/30 hover:bg-card/50 transition-colors">
              <CardHeader className="flex flex-row items-center gap-4 space-y-0 pb-4">
                ...
                <div className="flex-1 min-w-0">
                  <CardTitle className="text-lg font-bold truncate tracking-tight">{guild.name}</CardTitle>
                  <CardDescription className="font-mono text-[10px] uppercase opacity-70 flex items-center gap-2">
                    {guild.id}
                    <Badge variant="secondary" className="text-[10px] px-1.5 py-0 h-4">Initialized {new Date(guild.initialized_at).toLocaleDateString()}</Badge>
                  </CardDescription>
                </div>
                <Button 
                  variant="ghost" 
              ...
              <CardContent className="space-y-4">
                 <div className="space-y-3">
                   <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Enabled Modules</Label>
                   <div className="grid grid-cols-2 gap-2">
                     {["verify", "territories", "reaction_roles", "revive", "assist"].map(modId => (
                        <div key={modId} className="flex items-center space-x-2 bg-background/50 p-2 rounded-lg border border-border/50">
                          <Checkbox 
                            id={`${guild.id}-${modId}`} 
                            checked={guild.enabled_modules.includes(modId)}
                            onCheckedChange={(c) => toggleModule(guild.id, modId, !!c)}
                          />
                          <Label 
      ...
      <AlertDialog open={!!teardownId} onOpenChange={(o) => !o && setTeardownId(null)}>
        <AlertDialogContent className="bg-card/95 border-border shadow-2xl backdrop-blur-xl">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-xl font-bold">Teardown Guild Config?</AlertDialogTitle>
            <AlertDialogDescription>
              This will remove all configuration data for this guild from the database. 
              The bot will no longer function in this guild until initialized again. 
