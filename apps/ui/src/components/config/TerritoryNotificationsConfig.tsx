...
export const TerritoryNotificationsConfig = forwardRef(({
  ...
  const [ttFullChannelId, setTtFullChannelId] = useState<string>(initialData?.tt_full_channel_id || "");
  const [ttFilteredChannelId, setTtFilteredChannelId] = useState<string>(initialData?.tt_filtered_channel_id || "");
  const [ttTerritoryIds, setTtTerritoryIds] = useState<string>(((initialData?.tt_territory_ids || []) as string[]).join(", "));
  const [ttFactionIds, setTtFactionIds] = useState<string>(((initialData?.tt_faction_ids || []) as number[]).join(", "));
  ...
  useEffect(() => {
    ...
    const origFactions = [...(config.tt_faction_ids || [])].map(String).sort();
  ...
  }, [ttFullChannelId, ttFilteredChannelId, ttTerritoryIds, ttFactionIds, config, onDirtyChange]);
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      const fIds = parseIds(ttFactionIds).map(n => Number.parseInt(n)).filter(n => !Number.isNaN(n));
        ...
        <Card className="border-border/50 bg-card/50 backdrop-blur-sm hover:shadow-xl transition-shadow duration-300">
          <CardHeader>
            <div className="flex items-center gap-2 text-primary mb-1">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-primary/70">Global Tracker</span>
            </div>
            <CardTitle className="text-foreground">Full Notifications</CardTitle>
          ...
          <CardContent className="space-y-6">
            <div className="space-y-3">
              <Label className="text-[10px] uppercase tracking-wider font-black text-muted-foreground">Notification Channel</Label>
              <Select value={ttFullChannelId || "none"} onValueChange={(v) => setTtFullChannelId(v === "none" ? "" : v)}>
        ...
        <Card className="border-border/50 bg-card/50 backdrop-blur-sm hover:shadow-xl transition-shadow duration-300">
          <CardHeader>
            <div className="flex items-center gap-2 text-primary mb-1">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-primary/70">Filtered Alerts</span>
            </div>
            <CardTitle className="text-foreground">Filtered Notifications</CardTitle>
            <CardDescription className="text-muted-foreground/80">
              Receive updates for specific territories or factions you want to track.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-3">
              <Label className="text-[10px] uppercase tracking-wider font-black text-muted-foreground">Notification Channel</Label>
              <Select value={ttFilteredChannelId || "none"} onValueChange={(v) => setTtFilteredChannelId(v === "none" ? "" : v)}>
                <SelectTrigger className="w-full h-10 font-bold bg-background border-border/50 hover:bg-accent/50 transition-all">
                  <SelectValue placeholder="Select a channel" />
                </SelectTrigger>
                <SelectContent className="bg-background border-border">
