...
export async function handleTerritoryAlert(
  ...
  try {
    ...
    const configs = await db.guildConfig.findMany({
      where: {
        enabledModules: { has: "territory" },
        OR: [
          { ttFullChannelId: { not: null } },
          { ttFilteredChannelId: { not: null } },
        ],
      },
    });
    ...
    const territoryCode: string = data.tt || data.id || "";
    ...
    const ttLink = territoryCode
      ...
      : "*Unknown*";
    const assaultingFactionId: number | null = data.assaultingFaction || null;
    const defendingFactionId: number | null = data.defendingFaction || null;
    const victorFactionId: number | null = data.victorFaction || null;
    const factionId: number | null = data.factionId || null;
