...
export async function execute(
  ...
  try {
    ...
    for (const row of ownershipRows) {
      ...
      const allianceName = factionToAlliance.get(row.factionId) ?? "Unaligned";
    ...
    const totalControlled = ownershipRows.filter((row) => row.factionId !== null).length;
    ...
    const embed = createBaseEmbed(
      ...
      .addFields(
        { name: "Territories", value: `${ownershipRows.length}`, inline: true },
        { name: "Occupied", value: `${totalControlled}`, inline: true },
        { name: "No Owner", value: `${neutralCount}`, inline: true },
        { name: "Alliances Visible", value: `${rankedAlliances.length}`, inline: true },
      );
