...
export const data = new SlashCommandBuilder()
  ...
  .setDescription(
    "Revoke a user's ability to generate web magic links and clear active sessions",
  )
...
export async function execute(
  ...
  try {
    await magicLinkService.revokeUser(
      targetUser.id,
      interaction.user.id,
      reason,
    );
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `Successfully revoked web access for <@${targetUser.id}>. All active sessions have been terminated.`,
      )
