...
export async function execute(
  ...
  try {
    ...
    // Fetch faction name from cache
    const factionObj = await db.faction.findUnique({
      where: { id: factionId },
    });
    ...
    const ttState = await db.territoryState.findUnique({
      where: { id: territoryId },
    });

    // Fetch war ledger history (last 90 days)
    ...
    if (ttState) {
      const ownerStr = ttState.factionId
        ...
        : "*Unclaimed*";
      embed.addFields({ name: "Current Owner", value: ownerStr, inline: true });
    }
    ...
    // Get current owned territory count for faction
    const ownedTerritories = await db.territoryState.count({
      where: { factionId },
    });
    ...
    // Check specific territory war history for 72h cooldowns
    ...
    const errEmbed = createErrorEmbed("Error executing assault-check", errMsg);
