...
import { saveUserMap } from "@/actions/tt-selector";
import { useTransition } from "react";
import type { TerritoryMetadataResponse } from "@sentinel/shared";
...
interface TTSelectorProps {
  initialMaps?: UserMap[];
  metadata?: TerritoryMetadataResponse;
}
...
export function TTSelector({
  ...
  const [currentMap] = useState<UserMap>(() => {
    return initialMaps && initialMaps.length > 0
      ? initialMaps[0]
      : { name: "New Map", labels: DEFAULT_LABELS, assignments: {} };
  });

  const [labels, setLabels] = useState(currentMap.labels);
  ...
  const stats = useMemo(() => {
    ...
    labels.forEach((l) => {
      ...
      const livePrices: Record<string, number> = {
        ...(metadata?.prices?.items || {}),
        Points: metadata?.prices?.points || 0,
      };

      l.territories.forEach((territoryId) => {
        const bp = metadata?.territories?.[territoryId];
        aggregate[l.id].count += 1;
        aggregate[l.id].respect += bp?.respect || 0;
        aggregate[l.id].territories.push(territoryId);

        if (bp?.sector) {
          aggregate[l.id].sectors[bp.sector] =
            (aggregate[l.id].sectors[bp.sector] || 0) + 1;
        }
        ...
        if (bp?.racket) {
          ...
          if (rewardInfo) {
            ...
            aggregate[l.id].rewards.push(rewardInfo.displayString);
          }
        }
      });
    });
  ...
  useEffect(() => {
    ...
    const map = L.map(containerRef.current, {
      ...
      preferCanvas: true,
      renderer: L.canvas({ padding: 0.5 }), // Pre-render slightly outside view for smooth dragging
    });
    mapRef.current = map;
    ...
    const initSvg = async () => {
      ...
      try {
        ...
        const tornMapRawSVG = await res.text();
        ...
        // Fix relative image URLs natively in the SVG string before parsing!
        processedSVG = processedSVG.replace(
        ...
        // Strip out script tags from SVG
        rootNode.querySelectorAll("script").forEach((script) => script.remove());
        ...
        // Strip inline styles that cause CSP or performance issues
        svgElement
      ...
      try {
        ...
        svgPaths.forEach((path) => {
          ...
          const meta = metadata?.territories;
      ...
      const timer = setTimeout(() => {
        startTransition(async () => {
          try {
            ...
            toast.error("Failed to auto-save map.");
          }
        });
      }, 2000);
      return () => clearTimeout(timer);
    }
  }, [assignments, labels, initialPathsReady, currentMap.id, currentMap.name]);
  ...
  const handleManualSave = async () => {
    try {
      ...
      console.error("[TTSelector] Manual save failed:", err);
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    className="h-6 px-2 text-[10px] uppercase font-bold border border-white/20 text-zinc-300 hover:text-white hover:bg-white/10"
                  >
                          ...
                          <div className="p-3 rounded-b-xl bg-black/40 border-x border-b border-white/5 space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
                            ...
                            {labelStats.dailyValue > 0 && (
                              <div className="space-y-1.5">
                                ...
                                <div className="flex items-center gap-2 text-emerald-400 font-mono text-[11px] font-bold bg-emerald-500/5 border border-emerald-500/10 rounded-lg px-2.5 py-1.5 mb-1.5">
