...
export async function getGuildApiKeys(
  ...
  Array<{ id: string; apiKey: string; userId: string; isPrimary: boolean }>
> {
...
export const tornApi = {
  async get(
    endpoint: string,
    options: { apiKey: string; queryParams?: Record<string, any> },
  ) {
    ...
    const response = await fetch(
      `https://api.torn.com${endpoint}?key=${options.apiKey}&...`,
    );
