...
import { IpcClient, DEFAULT_IPC_SOCKET_PATH } from "../packages/utils/src/ipc.js";
...
function getSocketPath(): string {
  return process.env.IPC_SOCKET_PATH ?? DEFAULT_IPC_SOCKET_PATH;
}
 ...
 * the saved cursor position before each redraw. This avoids the scroll
 * desync bug that relative `up(N)` causes when the initial render scrolls
...
function renderAll(
  ...
  // Jump back to the saved position (set once before the first render)
  w(restoreCursor);
...
async function multiSelectWorkers(
  ...
  return new Promise((resolve) => {
    ...
    w(hideCursor);
    // Save the cursor position ONCE before the first render so every
    // subsequent render can jump back here with restoreCursor instead
    // of relying on relative up(N) which breaks when the terminal scrolls.
    w(saveCursor);
    render();
...
async function main() {
  ...
  if (schedules.length === 0) {
    ...
    w(`${dim}Ensure @sentinel/worker has booted at least once.${reset}\n\n`);
    await db.$disconnect();
    process.exit(1);
  }
