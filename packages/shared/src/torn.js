    ...
    async get(path, options) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            try {
                ...
                lastError = error;
                const isNetworkOrTimeout = error instanceof TypeError ||
...
ApiKeyRotator.poolIndices = new Map();
/**
 ...
 */
export function getNextApiKey(guildId, keys) {
    if (keys.length === 1) {
        return keys[0];
    }
