...
import { registerApiKeyUser, setApiKeyCooldown, setUserCooldown, } from "./api-key-cooldown.js";
...
export class PerUserRateLimiter {
    constructor(config) {
        ...
        this.unmappedKeysWarned = new Set(); // Track warned keys to avoid spam
        this.requestCounter = 0; // Counter for logging every Nth request
        this.initializationTime = Date.now(); // Track when rate limiter was created
        this.startupGracePeriodMs = 30000; // 30-second grace period after startup to allow entries to settle
        this.lastCleanupTime = 0;
        this.cleanupIntervalMs = 10000; // clean up at most once every 10 seconds
        this.db = config.db;
